function [soln, capital] = valueCRRA2(gridk, nk, maxit, tol, alpha, beta, A, sigma, k0)
    ...
    while it<=maxit && diff>tol;
        ...
        for kc=1:nk;
            vfun(it,kc) = 0;
            for kcc=1:nk;
                vhelp = ((A*gridk(kc)^alpha-gridk(kcc))^(1-sigma)-1)/(1-sigma)+beta*vfun(it-1,kcc);
                if vhelp>=vfun(it,kc)
                    vfun(it,kc) = vhelp;
    ...
    for kc=1:nk
        for kcc=1:nk
            table(kc,kcc) = ((A*gridk(kc)^alpha-gridk(kcc))^(1-sigma)-1)/(1-sigma)+beta*vfun(end,kcc);
        end;
    end;
